...
void readLineFromFd( int fd, char * buff, int buff_size) {
    char * ptr = buff;
    // **Update: Initializing the counter
    int counter = 1;
    while(1) {
        // **Update: Comparing counter to buffer size -1 and leaving space for /0.
        if(counter > buff_size -1 ) break;
...
int main( int argc, char ** argv)
{
    ...
    while(1) {
        ...
        // read response from socket **Update : sending buffer size
